...
# All rights reserved.
#
# Dr. Zero Biomedical Adaptation - Iteration 1 Challenger Training
# Trains proposer (question generator) on PubMed biomedical literature
...
CHALLENGER_NAME="biomed_challenger_iter1_ratio${hop_ratio}_${algorithm}_group${grpo_group_size}-${reward_group_size}_${model_name}"
...
if [ ! -f "${CORPUS_PATH}/pubmed-corpus.jsonl" ]; then
    ...
    echo "Please run: python scripts/download.py --corpus_type=pubmed --save_path=${CORPUS_PATH} --email=YOUR_EMAIL"
    exit 1
fi
...
echo "🧬 Starting biomedical retrieval server..."
...
# Launch base model solver for iteration 1
echo "🤖 Starting solver server (base model)..."
...
echo "🚀 Starting biomedical challenger training (iteration 1)..."
